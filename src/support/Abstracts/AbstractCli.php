...
abstract class AbstractCli extends PHPCLI
{
    ...
    protected function setup(Options $options)
    {
        ...
        foreach($this->moreCommands ?? [] as $commandName => $details) {
            $options->registerCommand($commandName, $details['help']);
            foreach($details['options'] ?? [] as $option) {
                $option[4] = $commandName;
    ...
    protected function runMore($command, Options $options)
    {
        if (in_array($command, array_keys($this->moreCommands), true)) {
            $callback = $this->moreCommands[$command]['callback'] ?? [$this, $command];
             if (is_callable($callback)) {
                call_user_func_array($callback, [$options->getOpt()]);
            }
            exit(0);
        }
        return null;
